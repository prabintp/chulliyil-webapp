        ...
        <div className="flex items-center justify-between gap-6">
          <h4 className="select-none text-lg font-bold tracking-tight my-4 transition duration-150 ease-out transform">
            ...
            <Link href="/" className="flex gap-1 items-center whitespace-nowrap tracking-[.002em]">
              {logoImage.src ? (
                <img src={logoImage.src} alt={logoImage.alt} className="h-20 w-auto" />
              ) : (
                <Icon
          ...
          <div className="block sm:hidden">
            <button onClick={toggleMobileMenu} className="text-gray-500 focus:outline-none focus:text-gray-700">
              <svg className="h-6 w-6 fill-current" viewBox="0 0 24 24">
                {isMobileMenuOpen ? (
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M19 15H5v-2h14v2zM19 9H5V7h14v2z"
                  />
                ) : (
                  <path
                    fillRule="evenodd"
          ...
          {/* Desktop menu */}
          <ul className="hidden sm:flex gap-6 sm:gap-8 lg:gap-10 tracking-[.002em] -mx-4">
            {data.nav &&
              data.nav.map((item, i) => {
                const activeItem =
                (item.href === ""
          ...
          {/* Mobile menu */}
          {isMobileMenuOpen && (
            <div className="sm:hidden absolute top-0 left-0 w-full h-full bg-white">
                  ...
                  data.nav.map((item, i) => (
                    <li key={`${item.label}-${i}`}>
                      <Link
                        data-tina-field={tinaField(item, "label")}
                        href={`/${item.href}`}
                        className="block text-center text-base tracking-wide transition duration-150 ease-out hover:opacity-100"
                      >
                        {item.label}
                      </Link>
                    </li>
